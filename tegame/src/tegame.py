...
import torch
from encode import build_state_vector,generate_all_possible_features,decode_feature
...
class Tegame:
    ...
    def build_log(self,player,mandatory_move,card_idx,pile_idx,piles_to_avoid=None,is_combo=False):
        log={}
        log['hand']=self.hands[player].copy()
        log['piles']=deepcopy(self.piles)
        log['mandatory_move']=mandatory_move
        log['played']=[card_idx,pile_idx]
        log['piles_to_avoid']=piles_to_avoid
        log['is_combo']=is_combo
        return log
    
        # Main action in a player turn
        # perform several actions to understand which card to play
    def play_card(self, player, mandatory_move=True):
        
        if self.train_mode: log=[]
            
        # Scan hand for best card
        delta_1, (card_idx1, pile_idx1) = self.scan_hand(player, mandatory_move=mandatory_move)
        if delta_1==100 and self.train_mode: log.append(self.build_log(player,mandatory_move,None,None))
        ...
        # Play combo cards if applicable
        combo_played=False
        for combo in combo_list:
            if card_idx1 in combo:
                combo_played = [self.hands[player][c] for c in combo]
                if self.verb_lvl>=3: print("\n COMBO!")
                if pile_idx1 > 1:
                    if self.train_mode: log.append(self.build_log(player,mandatory_move,combo[0], pile_idx1,is_combo=True))
                    self.place_card(player, combo[0], pile_idx1)
                    combo_played=True
                else:
                    if self.train_mode: log.append(self.build_log(player,mandatory_move,combo[-1], pile_idx1,is_combo=True))
        ...
        if not combo_played:
            # Special -10 card
            if delta_1 == -10:
                if self.train_mode: log.append(self.build_log(player,mandatory_move,card_idx1, pile_idx1))
                self.place_card(player, card_idx1, pile_idx1)
    
            # Non-mandatory move that can be skipped
            elif not mandatory_move and delta_1 > self.thresh_nonmandatory:
                if self.train_mode: log.append(self.build_log(player,mandatory_move,None,None))
    
            else:
                # Check interaction to avoid stepping on others
                dont_pile_idx_list = self.interaction(player)    
                
                print('here',dont_pile_idx_list)

                # Case 1: pile can be played
                if pile_idx1 not in dont_pile_idx_list:
                    if self.verb_lvl>=3: print("\nNo conflict detected!\n.........")
                    if self.train_mode: log.append(self.build_log(player,mandatory_move,card_idx1, pile_idx1,piles_to_avoid=dont_pile_idx_list))
                    self.place_card(player, card_idx1, pile_idx1)
                    
                # Case 2: all piles taken and move not mandatory
                elif sorted(dont_pile_idx_list) == [0, 1, 2, 3] and not mandatory_move:
                    if self.verb_lvl>=3: print("\nNo other choice..\n.........")
                    if self.train_mode: log.append(self.build_log(player,mandatory_move,card_idx1, pile_idx1,piles_to_avoid=dont_pile_idx_list))
                    self.place_card(player, card_idx1, pile_idx1)
            
                # Case 3: check alternative piles
                else:
                    ...
                    if delta_2 - delta_1 > self.thresh_secondchoice or delta_2 == 100:
                        ...
                        if delta_2 > self.thresh_nonmandatory and not mandatory_move:
                            if self.train_mode: log.append(self.build_log(player,mandatory_move,None,None,piles_to_avoid=dont_pile_idx_list))
                        else:
                            if self.train_mode: log.append(self.build_log(player,mandatory_move,card_idx2, pile_idx2,piles_to_avoid=dont_pile_idx_list))
                            self.place_card(player, card_idx2, pile_idx2)
    
        # Return log if in training mode, otherwise behave normally
        return log if self.train_mode else None
    ...
    def play_turn(self,player):
        ...
        for _ in range(n_played):
            if not self.deck_empty: self.draw_one(player)
    ...
    def scan_hand(self,player,discards=[],mandatory_move=True):
        ...
        if ind==[]:
            ...
            raise ValueError("Output of scan_hand_ not recognized")
    ...
    def scan_hand_(self,player,discards=[],mandatory_move=True):
        ...
        if delta==100 and discards==[] and mandatory_move:
            if not is_empty_lists(self.hands): self.game_over = True
            self.game_ongoing = False
    ...
    def interaction(self,active_player):
        ...
        for player in players:
            ...
            if pile_idx is not None: pile_idx_list.append(pile_idx)

        pile_idx_list = list(set(pile_idx_list))
        if pile_idx_list==[None]:
            return []
        else:
            return pile_idx_list

    # Physical action of placing one card onto a pile
    def place_card(self,player,card,pile):
        if card is not None and pile is not None:
            if self.verb_lvl>=2: print(f"\nDECISION:\nPlayer {player} places card: {self.hands[player][card]} on {self.piles[pile][-1]} (pile {pile})")
    ...
    def can_play(self, card, pile_idx):
        """
        Check if a given card can be played on a specific pile
        according to game rules.
        """
        if card==0: return False
        top_card = self.piles[pile_idx][-1]  # ultima carta sulla pila
        if pile_idx < 2:  # piles 0,1: increasing
            return card > top_card or card == top_card - 10
        else:  # piles 2,3: decreasing
            return card < top_card or card == top_card + 10

    def get_mask_of_illegal_moves(self,hand):
        npiles = len(self.piles)
        ncards = len(hand)
        msk = torch.ones([ncards,npiles],dtype=torch.bool)
        for card_idx,card in enumerate(hand):
            for pile_idx,pile in enumerate(self.piles):
                if not self.can_play(card,pile_idx): msk[card_idx,pile_idx]=0
        return msk.view([ncards*npiles])
        
class TegameML(Tegame):
    """
    Subclass of Tegame that uses a trained policy network (PyTorch model)
    to select which card to play.

    Parameters
    ----------
    model : nn.Module
        Trained PyTorch model that predicts (card, pile) from state vector.
    *args, **kwargs :
        Arguments passed to the original Tegame constructor.
    """
    ...
    def interaction(self,player):
        ...
        for i in range(self.n_players):
            if not i==player:
                card, pile_idx = self.model_predict_move(player,mandatory_move=False)
                if not pile_idx in pile_list: pile_list.append(pile_idx)
        if self.verb_lvl>=3:
            print("\n------------")
            print("Interaction")
            print("------------")
            if pile_list!=[None]:
                print("The other players need to play on pile(s): ",pile_list)
            else:
                print("The other players do not need to urgently play any pile.")
        return pile_list
            
    def play_card(self, player, mandatory_move=True):
        """
        Play a card using the trained ML model policy.
        Simplified: ignores combos, special rules, and logging.
    
        Steps:
        1. Compute piles to avoid (interaction with other players).
        2. Build state vector for the model.
        3. Predict logits for cards and piles.
        4. Apply legal masks (including NO-OP) in float.
        5. Select the move using argmax on masked logits.
        6. Execute the move or do nothing if NO-OP.
        """

        # Stop if the game is over
        if self.game_over: return
                
        # Compute piles to avoid
        piles_to_avoid = self.interaction(player)
        card_idx,pile_idx=self.model_predict_move(player,mandatory_move,piles_to_avoid=piles_to_avoid)
        self.place_card(player, card_idx, pile_idx)
        
    def model_predict_move(self, player, mandatory_move, piles_to_avoid=None):
        hand = self.hands[player]
        
        features = generate_all_possible_features(hand, self.piles, mandatory_move, piles_to_avoid)
        ...
        # 5. Mask
        illegal_mask = self.get_mask_of_illegal_moves(hand)
        dim=X.shape[0]
        mask = build_action_mask(hand, mandatory_move, dim, illegal_mask)
    
        if not torch.any(mask):
            #case 1: there is no legal move and hands are not empty --> game over
            if mandatory_move and not is_empty_lists(self.hands):
                self.game_over = True
                self.game_ongoing = False
                return None, None
            #case 2: there is no legal move and hands are empty --> game win
            else:
                self.game_ongoing = False
                return None, None
        else: 
            #case 3: there are legal moves 
            ...
            piles_top=[pile[-1] for pile in self.piles]
            card_idx, pile_idx = decode_feature(best_feature,hand,piles_top)
...
def build_action_mask(hand, mandatory_move, dim, illegal_mask):
    mask = torch.zeros(dim, dtype=torch.bool)

    # NOOP
    mask[-1] = not mandatory_move

    # mosse reali
    n_real_moves = len(hand) * N_PILES
    mask[:n_real_moves] = illegal_mask
