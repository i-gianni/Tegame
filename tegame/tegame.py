...
class tegame:
    ...
    def scan_combo(self,player):
        ...
        #sort_indeces = argsort(self.hands[player])
        #sort_indeces_inv = inverse_permutation(sort_indeces)
    ...
    def play_card(self,player,mandatory_move=True):
        ...
        for combo in combo_list:
            if card_idx1 in combo:
                if not mandatory_move and delta_1 > self.thresh_nonmandatory:
                    break
                else:
                    print(f"\nCCCCCCCCCCOMBO: {" ".join(str(_)for _ in combo)}")
                    for card in reversed(combo):
                        self.place_card(player,card,pile_idx1)
                    return
